...
export function firstViewImage(side, routeId = "concept") {
  ...
  const media = responsiveImageManifest[portrait.src];
  const srcset = media ? ` srcset="${media.candidates.map(c => `${c.src} ${c.width}w`).join(", ")}" sizes="(max-width: 767px) 100vw, 50vw"` : "";
  return `<img src="${media?.src || portrait.src}"${srcset} alt="${portrait.alt}" width="${portrait.width}" height="${portrait.height}" fetchpriority="high" decoding="async" data-concept-fv-image style="display:block;width:100%;height:100%;object-fit:cover;object-position:${portrait.position};transform:scale(${portrait.scale});transform-origin:${portrait.origin};">`;
}
